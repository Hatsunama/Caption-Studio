...
export function WatermarkSheet(props: Props) {
  ...
  useEffect(() => { if (!props.visible) setText(''); }, [props.visible]);
  return (
      ...
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: '#000000A6' }}>
        <View style={{ maxHeight: '76%', gap: 14, borderTopLeftRadius: 24, borderTopRightRadius: 24, borderWidth: 1, borderColor: '#FF8FC455', backgroundColor: chrome.surface, padding: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Pressable accessibilityLabel="Close watermarks" onPress={props.onClose} style={{ padding: 8 }}><Text style={{ color: chrome.muted, fontSize: 14, fontWeight: '800' }}>CLOSE</Text></Pressable>
          </View>
          ...
          <Pressable accessibilityLabel="Add watermark" disabled={atLimit || !text.trim()} onPress={() => props.onAdd(text.trim())} style={{ alignItems: 'center', borderRadius: 12, backgroundColor: atLimit || !text.trim() ? '#6D526088' : '#E8579C', paddingVertical: 13 }}><Text style={{ color: '#170C13', fontSize: 14, fontWeight: '900' }}>{atLimit ? 'FIVE WATERMARKS ADDED' : 'ADD WATERMARK'}</Text></Pressable>
