...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.31) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.31/caption-studio-android.apk
...
Current Android build: **1.4.31** (`v1.4.31`, Android version code 43).

Version 1.4.31 makes video-clip reordering topology-preserving for subtitles. Each source-owned primary caption moves by the exact timeline delta of its owning clip without reconstruction, while its identity, text, word references, style, and secondary-language association remain intact. Timeline-owned captions stay in place. It also includes the 1.4.30 transition playback, on-canvas selection, authored-newline, resize, and topology-only split repairs.
...
The installer requires release 1.4.31 or newer and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
