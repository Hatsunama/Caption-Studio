...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.46) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.46/caption-studio-android.apk
...
Current Android build: **1.4.46** (`v1.4.46`, Android version code 58).

Version 1.4.46 makes caption editing a revision-safe session, keeps live bilingual previews aligned with independently timed tracks, preserves portrait canvas geometry, and keeps companion preview media admitted during script playback. It also virtualizes long dual-caption editors, clears recovery journals when a project is discarded or deleted, preserves successful video exports when optional sharing is unavailable, corrects timeline-gap translation timing, and makes fade-dark export rendering explicit.

Version 1.4.42 keeps caption editing in a lower-half in-editor sheet so the preview and playback control remain usable. A centered neon-green timing guide links the caption list and video transport in both directions. Caption generation now routes to Captions, bulk caption changes never require a selected timeline block, and cue-aware emoji animations use fewer reactions while spanning the full cue duration after caption edits, splits, joins, or timing changes.
...
The installer requires release 1.4.46 or newer and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
