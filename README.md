...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.17-fixed.1) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-fixed-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.17-fixed.1/caption-studio-fixed-android.apk
...
Current fixed build: **1.4.17** (`v1.4.17-fixed.1`, Android version code 29).

Version 1.4.17 preserves integrity-bound caption and translation model downloads across Android foreground interruptions, reserves storage only for remaining resumable bytes, and reports in plain English that retry continues saved progress. Lifecycle decisions remain in UI controllers, transfer and storage policy remain in services, and release identity remains outside presentation code. The production LiteRT-LM model remains unchanged while the Natural multilingual v2 candidate completes its strict model and two-phone gates.
...
The installer requires the 1.4.17 repair tag and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
