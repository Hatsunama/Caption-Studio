...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.43) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.43/caption-studio-android.apk
...
Current Android build: **1.4.43** (`v1.4.43`, Android version code 55).

Version 1.4.43 removes retry amplification from local multilingual translation. Each valid cue fragment checkpoints independently, malformed or oversized input is reported rather than silently lost, and response budgets match the requested cue shape. Every supported language may be the primary spoken-caption language and translate to any different supported language. Manual caption line breaks are literal user edits: Enter may split text anywhere, including inside joined words.
...
The installer requires release 1.4.43 or newer and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
