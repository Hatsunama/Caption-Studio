...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.32) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.32/caption-studio-android.apk
...
Current Android build: **1.4.32** (`v1.4.32`, Android version code 44).

Version 1.4.32 gives every layer-order and layer-removal action a visible 26 px control, an 18 px glyph, a centered button surface, and an explicit accessibility label while keeping all three controls inside the lane header. It also makes video-clip reordering topology-preserving for subtitles: each source-owned primary caption moves by the exact timeline delta of its owning clip without reconstruction, while its identity, text, word references, style, and secondary-language association remain intact. Timeline-owned captions stay in place.
...
The installer requires release 1.4.32 or newer and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
