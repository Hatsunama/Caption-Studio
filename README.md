...
1. Open the [latest Caption Studio release](https://github.com/Hatsunama/Caption-Studio/releases/tag/v1.4.30) on the phone.
...
curl -L -o ~/storage/downloads/caption-studio-android.apk \
  https://github.com/Hatsunama/Caption-Studio/releases/download/v1.4.30/caption-studio-android.apk
...
Current Android build: **1.4.30** (`v1.4.30`, Android version code 42).

Version 1.4.30 consolidates duplicate directional transitions into one Push family and migrates saved Wipe/Slide choices to the matching direction. Composite previews preload their two decoders before playback reaches the boundary, render first frames while hidden, avoid repeated decoder seeks during playback, and never cover the video with a loading banner. Transition timing now opens from one bottom-sheet control with choices from 0.2 to 2 seconds. Preview taps select captions, translations, text, and images directly. Text layers preserve every authored newline as a non-wrapping line while fitting type to the resized box, and transform handles use an immutable gesture center. Video splitting is topology-only: subtitle IDs, text, timing, word references, styles, and translation associations stay unchanged.
...
The installer requires release 1.4.30 or newer and refuses older APKs while the release is building. Both download routes use Hatsunama/Caption-Studio. See [audit coverage](docs/audit-1.4.8.md).
