...
- First caption generation downloads one immutable, hash-pinned multilingual Whisper tiny model and one Silero VAD speech-detection model from Hugging Face. Optional dual-language translation downloads one immutable, hash-pinned Qwen model of approximately 1.6 GB from Hugging Face. Media, transcripts, and translations are not included in those requests; ordinary connection information such as IP address is necessarily exposed to Hugging Face.
- Natural translation across supported caption-language pairs runs entirely on-device after the optional model download. No Google Translation SDK or translation backend is used.
